...
class format_the_output(unittest.TestCase):
    def test_basic_format(self):
        self.sut = pexpect.spawnu(
            "python -m colorize -f 'a%(message)sa' -- echo FAILURE", timeout=3
        )
    ...
    def test_just_stdout(self):
        self.sut = pexpect.spawnu(
            "python -m colorize -f 'b%(message)sb' -- echo FAILURE", timeout=3
        )
    ...
    def test_normal_output(self):
        self.sut = pexpect.spawnu(
            "python -m colorize -f 'b%(message)sb' -- echo foo", timeout=3
        )
    ...
    def test_format_date(self):
        ...
        self.assertEquals(time.strftime(format), time.strftime(format, parsed))
